package go.pemkott.appsandroidmobiletebingtinggi.ui.rekap;
...
import go.pemkott.appsandroidmobiletebingtinggi.databinding.FragmentRekapBinding;
...
public class RekapFragment extends Fragment {
    ...
    DialogView dialogFragment = new DialogView(getActivity());
    private FragmentRekapBinding binding;
    ...
    public View onCreateView(@NonNull LayoutInflater inflater,
        ...
        rvRekapServer= binding.rvRekapServer;

        LinearLayout llSampaiRekap = binding.llSampaiRekap;
        LinearLayout llMulaiRekap = binding.llMulaiRekap;
        tvHariMulaiRekap = binding.tvHariMulaiRekap;
        tvBulanTahunMulaiRekap = binding.tvBulanTahunMulaiRekap;
        tvtTampilkanRekap = binding.tvtTampilkanRekap;

        tvBulanTahunSampaiRekap = binding.tvBulanTahunSampaiRekap;
        tvHariSampaiRekap = binding.tvHariSampaiRekap;
        ...
        tvtTampilkanRekap.setOnClickListener(v -> {
            ...
            if(NetworkUtils.isConnected(getActivity())){
                ...
                if (dariTanggal == null || sampaiTanggal == null){
                    ...
                    dialogFragment.viewErrorKosong(getActivity());
                    ...
                    if (dari.getTime() > hariIni.getTime()){
                        dialogFragment.viewNotifKosong(getActivity(), "Rekap tidak tersedia pada waktu yang anda tentukan.", "");
                    }else{
                        assert sampai != null;
                        if (sampai.getTime()<dari.getTime()){
                            Toast.makeText(getActivity(), "Pemilihan tanggal tidak boleh terbalik", Toast.LENGTH_SHORT).show();
                        }else{
    ...
    public void dataRekapServer(String idE, String dariTanggal, String sampaiTanggal){
        ...
        callRekapAbsensi.enqueue(new Callback<List<RekapServer>>() {
            ...
            public void onResponse(Call<List<RekapServer>> call, Response<List<RekapServer>> response) {
                ...
                rvRekapServer.setLayoutManager(new LinearLayoutManager(getActivity()));
                rekapServerAdapter = new RekapServerAdapter(rekapServers, getActivity());
    ...
    public void viewPresensi(RekapServer data){
        ...
        persyaratanDialog = new Dialog(getActivity(), R.style.DialogStyle);
...
//        tvAktivitasRekapMasuk.setText(data.getKet_masuk());
        ...
        if (data.getLat_masuk() == null || data.getLat_masuk().isEmpty()){
            ...
            tvLokasiAlamatLuarKantor.setText(getAddress(getActivity(), Double.parseDouble( data.getLat_masuk()), Double.parseDouble(data.getLng_masuk())));
        }
        ...
        if (data.getLat_pulang() == null || data.getLat_pulang().isEmpty()){
            ...
            tvLokasiAlamatLuarKantorPulang.setText(getAddress(getActivity(), Double.parseDouble( data.getLat_pulang()), Double.parseDouble(data.getLng_pulang())));
        }
    ...
    public void pesanError(){
        Dialog errorDialogs = new Dialog(requireActivity(), R.style.DialogStyle);
    ...
    private void datePickerMulai(){
        ...
        datePickerDialogMulai = new DatePickerDialog(requireActivity(), android.R.style.Theme_DeviceDefault_Light_Dialog, dateSetListener, tahun, bulan, hari);
    }
...
//        int style = AlertDialog.THEME_HOLO_LIGHT;
        ...
        datePickerDialogSampai = new DatePickerDialog(requireActivity(), android.R.style.Theme_DeviceDefault_Light_Dialog, dateSetListener, tahun, bulan, hari);
    }
